...
object Iris_LR {
  def main(args: Array[String]) {
    ...
    //
    val hpcc = new HpccFile(hpcc_file, hpcc_protocol, hpcc_ip, hpcc_port, "", "")
    val parts = hpcc.getFileParts()
    val rd = hpcc.getRecordDefinition()
    val myRDD = new HpccRDD(sc, parts, rd)
